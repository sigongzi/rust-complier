...
impl FunctionInfo {
    ...
    pub fn add_variable_name(&mut self, name: String, val: Value) {
        self.variable_name.insert(name, val);
    }

    // require if the val is in const hashset 
    pub fn require_variable_name(&mut self, name: &str) -> Option<Value>{
        self.variable_name.get(name).copied()
    }
}